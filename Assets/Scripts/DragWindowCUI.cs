...
namespace CurvedUI
{
    ...
    /// </summary>
    public class DragWindowCUI : MonoBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler
    {
        ...
        
        {
            ...
            else if (CurvedUIInputModule.ControlMethod == CurvedUIInputModule.CUIControlMethod.STEAMVR_2)
            {
                // CurvedUIInputModule.Hand currHand = CurvedUIInputModule.Instance.UsedHand;
                Transform controllerTransform = CurvedUIInputModule.Instance.ControllerTransform;
                //get direction of the controller
                Ray controllerRay = new Ray(controllerTransform.position, controllerTransform.forward);
                //position when using gaze - uses the center of the screen as guiding point.
                GetComponentInParent<CurvedUISettings>().RaycastToCanvasSpace(controllerRay, out newPos);
            }
